...
/** Realistic stateful DOM stress fixture adapted from Delivery Market's MobileJourneyFaqStrip. */
export function DeliveryMarketFaqFixture() {
  ...
  return (
    <section className="delivery-faq-stage" aria-label="Delivery Market FAQ capture stress fixture">
      <div className="delivery-faq-copy"><p>REAL APPLICATION DOM</p><h2>Delivery Market FAQ strip</h2><span>Scroll this stateful component beneath the fixed glass.</span></div>
      <div data-journey-faq className="delivery-faq-strip">
        <button type="button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} className="delivery-faq-trigger">
          <span className="delivery-faq-label">FAQ</span><span className="delivery-faq-question">{items[active]?.question}</span><span className="delivery-faq-count">{active + 1}/{items.length}</span><span className="delivery-faq-chevron" aria-hidden="true">{expanded ? "⌃" : "⌄"}</span>
        </button>
        {expanded ? <div className="delivery-faq-expanded">{items.map((item) => <div key={item.question} className="delivery-faq-item"><p>{item.question}</p><span>{item.answer}</span></div>)}</div> : null}
      </div>
    </section>
  );
}
